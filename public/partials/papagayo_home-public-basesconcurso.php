...
wp_nav_menu(
    array(
        'container'       => 'nav',
        'container_class' => 'cuerpoMenu col-md-12 col-sm-12 col-xs-12',
        'container_id'    => 'mnu',
        'items_wrap'      => '<ul>%3$s</ul>',
        'theme_location'  => 'basesconsurso'
    ) );

$args     = new WP_Query( array(
	'posts_per_page' => 6,
	'order'          => 'ASC',
	'post_type'      => 'basesconsurso',
	'post_status'    => 'publish'
) );
...
if ( $contar % 2 == 0 ) {
    ...
    <div>
        <div class="est col-md-12 col-sm-12 col-xs-12">
	        ...
	        ?>
	        <?php if ( $meta_print_boton ) { ?>
                <button>
                    <a <?php if ($meta_print_boton2)
                    {?>data-toggle="modal" data-target="#myModal"<?php }?>
                    href="<?php echo $meta_print_url ?>"
                    >
				        <?php echo $meta_print_boton ?>
                    </a>
                </button>
	        <?php } ?>
        </div>
